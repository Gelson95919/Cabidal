...
class planconta(models.Model):
     ...
     def exist_cont(self):
         ...
         if self.cod_plan_cont and self.cod_plan_cont in exsist_cont:
             raise ValidationError('Conta Duplicado!')

     @api.onchange('cod_plan_cont', 'estruturas_Contas')
     def valida_estrot_cont(self):
         if self.cod_plan_cont:
             ...
             if self.novo_plan == False:
                 ...
                 if item == 1:
                     plano = self.env['planconta.planconta'].search([('mae', '=', True)])
                     plano.mae = False
                     self.exist_mae = True  # e a conta mae
                     self.mae = True
                 list_de_x = []
                 estrot_dif = element.split('.')
                 for e in estrot_dif:
                     list_de_x.append(e)

                 if list_de_x[0] == 'X' and len_cont[0] =='1':
                     plano = self.env['planconta.planconta'].search([('cod_plan_cont', '=', '1')])
                     if plano:
                         pass
                     else:
                         raise ValidationError('esta conta ainda não pode ser criada')
                 elif list_de_x[1] == 'XX' and len_cont[0] =='1':
                      pass
                 #else:
                 #    pass
                     #list_mae = [] #len do compo cod_plan_cont
                     #estrot_pas = []  # len de compo estrotura passado
                     #count = 0
                     #for item in codig:
                     ...
                     #    self.conta_mae = list_mae[0]
                     #    plano = self.env['planconta.planconta'].search([('mae', '=', True)])

                     #    if plano:
                     #        est = plano.estruturas_Contas
                     #        est_ant = str(est)
                     #        for l in est_ant:
                     #            estrot_pas.append(l)
                     #        if len(list_mae) <= len(estrot_pas):
                     #           self.estruturas_Contas = plano.estruturas_Contas  #preenche o campo estrutura
                     #        else:
                     #            if not self.estruturas_Contas:
                     #                self.estruturas_Contas = ''
                     #                warning = {
                     #                  'title': _('Aviso!'),
                     #                  'message': _('Adicione um novo estrutura para esta conta!\n' + ' ' + 'Conta' + ' ' + str(codig) + ',' + ' ' + 'Estrutura maior  que a conta pai.'),
                     #                }
                     #                return {'warning': warning}

                                 #if self.estruturas_Contas:
                                 #    list_est_pres = []
                                 #    est_pres = self.estruturas_Contas
                                 #    estrut_prese = str(est_pres)

                                 #    estrot_dif = estrut_prese.split('.')
                                 #    estr_em_x = estrot_dif

                                 #    for est in estr_em_x:
                                 #        list_est_pres.append(est)


                                 #    if len(list_est_pres) >= len(list_mae):
                                 #        ex_mae = self.env['planconta.planconta'].search([('mae', '=', True)])
                                 #        ex_mae.mae = False
                                 #        ex_mae.grupo = True
                                 #        self.mae = True
                                 #    else:
     ...
     def chekStrutur(self):#identifica se a estrutura se e mae ou filho
         if self.cod_plan_cont:
             if self.novo_plan == False:
                  ...
                  if count > 1:#estrutura Filho      "Se o campo cod_plan_cont tem mais de que um digito"
                      self.conta_mae = cod_def_str[0]
                      plano = self.env['planconta.planconta'].search([('cod_plan_cont', '=', self.conta_mae)])
                      if plano:
                         self.estruturas_Contas = plano.estruturas_Contas #preenche o campo estrutura
                      else:
                          self.estruturas_Contas = ''
     ...
     def upStrutuConta(self):
         if self.estruturas_Contas or self.natureza_saldo:
             if self.novo_plan == False:
